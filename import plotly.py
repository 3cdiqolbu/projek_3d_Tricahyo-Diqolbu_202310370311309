...
import numpy as np
import json # Tetap diperlukan jika Anda menggunakan json.dumps di tempat lain, tapi untuk show() mungkin tidak.
...
def create_rocket_figure():
    ...
    fig.update_layout(
        title='3D Rocket Model (Local Run)', # Ganti judul agar jelas ini versi lokal
        scene=dict(
 ...
 # Z-axis is up
        ...
        # paper_bgcolor='rgba(0,0,0,0)', # Ini hanya relevan untuk embed di HTML
        # plot_bgcolor='rgba(0,0,0,0)'  # Ini juga
    )
    return fig

# Main execution block for local Python
if __name__ == "__main__":
    # Pastikan Plotly sudah terinstal: pip install plotly numpy
    fig = create_rocket_figure()
    fig.show() # Ini akan membuka browser web Anda dan menampilkan modelnya